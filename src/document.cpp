...
namespace treesheets {
	...
	bool Document::MyPrintout::OnPrintPage(int page) {
		wxDC *dc = GetDC();
		if (!dc) return false;
		doc->Print(*dc, *this);
		return true;
	...
	bool Document::MyPrintout::OnBeginDocument(int startPage, int endPage) {
		return wxPrintout::OnBeginDocument(startPage, endPage);
	}

	void Document::MyPrintout::GetPageInfo(int *minPage, int *maxPage, int *selPageFrom, int *selPageTo) {
		*minPage = 1;
		*maxPage = 1;
		*selPageFrom = 1;
		*selPageTo = 1;
	...
	bool Document::MyPrintout::HasPage(int pageNum) { return pageNum == 1; }
	...
	uint Document::Background() { return rootgrid ? rootgrid->cellcolor : 0xFFFFFF; }

	void Document::InitWith(Cell *r, wxString filename) {
		rootgrid = r;
	...
	void Document::UpdateFileName(int page) {
		g_sys->frame->SetPageTitle(filename, modified ? (lastmodsinceautosave ? L"*" : L"+") : L"",
			page);
	}

	void Document::ChangeFileName(const wxString &fn) {
		filename = fn;
		UpdateFileName();
	}
	...
	const char *Document::SaveDB(bool *success, bool istempfile, int page) {
		...
		{  // limit destructors
			wxBusyCursor wait;
			if (!istempfile && g_sys->makebaks && ::wxFileExists(filename)) {
				::wxRenameFile(filename, g_sys->BakName(filename));
			}
			wxString sfn = istempfile ? g_sys->TmpName(filename) : filename;
			...
			fos.Write(&vers, 1);
			loopv(i, g_sys->imagelist) g_sys->imagelist[i]->trefc = 0;
			rootgrid->ImageRefCount();
			int realindex = 0;
			loopv(i, g_sys->imagelist) {
				Image &image = *g_sys->imagelist[i];
				if (image.trefc) {
					fos.Write("I", 1);
		...
		if (!istempfile) {
			...
			modified = false;
			tmpsavesuccess = true;
			g_sys->FileUsed(filename, this);
			if (::wxFileExists(g_sys->TmpName(filename))) ::wxRemoveFile(g_sys->TmpName(filename));
		}
	...
	void Document::DrawSelectMove(wxDC &dc, Selection &s, bool refreshalways, bool refreshinstead) {
	...
	bool Document::ScrollIfSelectionOutOfView(wxDC &dc, Selection &s, bool refreshalways) {
	...
	void Document::ScrollOrZoom(wxDC &dc, bool zoomiftiny) {
	...
	void Document::ZoomTiny(wxDC &dc) {
	...
	void Document::ResetCursor() {
		if (selected.g) selected.SetCursorEdit(this, selected.TextEdit());
	}

	void Document::Hover(int x, int y, wxDC &dc) {
		if (redrawpending) return;
		ShiftToCenter(dc);
		ResetFont();
		...
		g_sys->UpdateStatus(hover);
	}

	char *Document::Select(wxDC &dc, bool right, int isctrlshift) {
		begindrag = Selection();
	...
	void Document::Zoom(int dir, wxDC &dc, bool fromroot, bool selectionmaybedrawroot) {
	...
	void Document::ShiftToCenter(wxDC &dc) {
	...
	void Document::Render(wxDC &dc) {
		...
		for (Cell *p = curdrawroot->parent; p; p = p->parent)
			if (p->text.t.Len()) {
				...
				wxString s = p->text.t;
				if ((int)s.Len() > g_sys->defaultmaxcolwidth) {
					...
					s = s.Left(g_sys->defaultmaxcolwidth) + L"...";
				}
	...
	void Document::Draw(wxDC &dc) {
		...
		centerx = g_sys->centered && !originx && maxx > layoutxs
			? (maxx - layoutxs) / 2 * currentviewscale
			: 0;
		centery = g_sys->centered && !originy && maxy > layoutys
	...
	void Document::Print(wxDC &dc, wxPrintout &po) {
		Layout(dc);
		maxx = layoutxs;
		maxy = layoutys;
		originx = originy = 0;
	...
	bool Document::FontIsMini(int textsize) { return textsize == g_mintextsize(); }

	bool Document::PickFont(wxDC &dc, int depth, int relsize, int stylebits) {
		...
		if (textsize != lasttextsize || stylebits != laststylebits) {
			dc.SetFont(
				wxFont(textsize - (while_printing || scaledviewingmode),
					...
					stylebits & STYLE_FIXED ? L"" : g_sys->defaultfont));
	...
	bool Document::CheckForChanges() {
		if (modified) {
			ThreeChoiceDialog tcd(g_sys->frame, filename,
	...
	bool Document::CloseDocument() {
		...
		if (!keep && !filename.empty() && ::wxFileExists(g_sys->TmpName(filename)))
			::wxRemoveFile(g_sys->TmpName(filename));
		return keep;
	}

	const char *Document::DoubleClick(wxDC &dc) {
	...
	const char *Document::ExportFile(const wxString &fn, int k) {
		if (k == A_EXPCSV) {
	...
	const char *Document::Key(wxDC &dc, wxChar uk, int k, bool alt, bool ctrl, bool shift,
		bool &unprocessed) {
		Cell *c = selected.GetCell();
		if (uk == WXK_NONE || (k < ' ' && k)) {
			switch (k) {
	...
	const char *Document::Action(wxDC &dc, int k) {
		ShiftToCenter(dc);
		...
		case A_IMPTXTC:
		case A_IMPTXTS:
		case A_IMPTXTT: return g_sys->Import(k);
		...
		case A_CLOSE: {
			if (g_sys->frame->nb->GetPageCount() <= 1) {
				g_sys->frame->fromclosebox = false;
				g_sys->frame->Close();
				return NULL;
			}

			if (!CloseDocument()) {
				int p = g_sys->frame->nb->GetSelection();
				// g_sys->frame->nb->AdvanceSelection();
				g_sys->frame->nb->DeletePage(p);
		...
		case A_HELPI: g_sys->LoadTut(); return NULL;

		case A_HELP:
#ifdef __WXMAC__
			wxLaunchDefaultBrowser(L"file://" +
				g_sys->frame->GetPath(L"docs/tutorial.html"));  // RbrtPntn
#else
			wxLaunchDefaultBrowser(g_sys->frame->GetPath(L"docs/tutorial.html"));
#endif
			return NULL;
		...
		case A_DEFFONT: {
			...
			fdat.SetInitialFont(wxFont(g_deftextsize, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL,
				wxFONTWEIGHT_NORMAL, false, g_sys->defaultfont));
			wxFontDialog fd(g_sys->frame, fdat);
			if (fd.ShowModal() == wxID_OK) {
				wxFont font = fd.GetFontData().GetChosenFont();
				g_sys->defaultfont = font.GetFaceName();
				g_deftextsize = min(20, max(10, font.GetPointSize()));
				g_sys->cfg->Write(L"defaultfont", g_sys->defaultfont);
				g_sys->cfg->Write(L"defaultfontsize", g_deftextsize);
				// rootgrid->ResetChildren();
				g_sys->frame->TabsReset();  // ResetChildren on all
				Refresh();
			}
			return NULL;
		}
		...
		case A_PRINT: {
			...
			if (printer.Print(g_sys->frame, &printout, true)) {
		...
		case A_PAGESETUP: {
			pageSetupData = printData;
			wxPageSetupDialog pageSetupDialog(g_sys->frame, &pageSetupData);
		...
		case A_NEXTFILE: g_sys->frame->CycleTabs(1); return NULL;
		case A_PREVFILE: g_sys->frame->CycleTabs(-1); return NULL;

		case A_CUSTCOL: {
			uint c = PickColor(g_sys->frame, g_sys->customcolor);
			if (c != (uint)-1) g_sys->customcolor = c;
			return NULL;
		}

		case A_DEFBGCOL: {
			uint oldbg = Background();
			uint c = PickColor(g_sys->frame, oldbg);
			if (c != (uint)-1) {
		...
		case A_ROUND5:
		case A_ROUND6:
			g_sys->cfg->Write(L"roundness", long(g_sys->roundness = k - A_ROUND0));
			Refresh();
			return NULL;
		...
		case A_CUSTKEY: {
			wxArrayString strs;
			MyFrame::MenuString &ms = g_sys->frame->menustrings;
		...
		case A_CUT:
		case A_COPY:
			DELETEP(g_sys->cellclipboard);
			g_sys->clipboardcopy = wxEmptyString;
		...
		case A_MARKCODE: {
			...
			loopallcellssel(c, false) {
				c->celltype =
					(newcelltype == CT_CODE) ? g_sys->ev.InferCellType(c->text) : newcelltype;
				Refresh();
			}
			return NULL;
		}
		...
		case A_PASTE:
			...
			else if (g_sys->cellclipboard) {
				c->Paste(this, g_sys->cellclipboard, selected);
				Refresh();
			}
			return NULL;
		...
		case A_COLCELL: g_sys->customcolor = c->cellcolor; return NULL;
	...
	uint Document::PickColor(wxFrame *fr, uint defcol) {
	...
	const char *Document::layrender(int ds, bool vert, bool toggle, bool noset) {
		if (selected.Thin()) return NoThin();
		selected.g->cell->AddUndo(this);
	...
	void Document::ZoomOutIfNoGrid(wxDC &dc) {
		if (!WalkPath(drawpath)->grid) Zoom(-1, dc);
	}

	void Document::PasteSingleText(Cell *c, const wxString &t) { c->text.Insert(this, t, selected); }

	void Document::PasteOrDrop() {
		Cell *c = selected.GetCell();
		...
		default:  // several text formats
			if (dataobjt->GetText() != wxEmptyString) {
				...
				if ((g_sys->clipboardcopy == s) && g_sys->cellclipboard) {
					c->Paste(this, g_sys->cellclipboard, selected);
					Refresh();
				}
				else {
					...
					if (as.size()) {
						if (as.size() <= 1) {
							...
							c->ResetLayout();
							DELETEP(c->grid);
							g_sys->FillRows(c->AddGrid(), as, g_sys->CountCol(as[0]), 0, 0);
							if (!c->HasText())
	...
	void Document::CreatePath(Cell *c, Vector<Selection> &path) {
		path.setsize(0);
		while (c->parent) {
	...
	Cell *Document::WalkPath(Vector<Selection> &path) {
		Cell *c = rootgrid;
		loopvrev(i, path) {
			Selection &s = path[i];
			Grid *g = c->grid;
	...
	bool Document::LastUndoSameCell(Cell *c) {
	...
	void Document::Undo(wxDC &dc, Vector<UndoItem *> &fromlist, Vector<UndoItem *> &tolist, bool redo) {
	...
	void Document::ColorChange(int which, int idx) {
		if (!selected.g) return;
		selected.g->ColorChange(
			this, which, idx == CUSTOMCOLORIDX ? g_sys->customcolor : celltextcolors[idx], selected);
	}

	void Document::SetImageBM(Cell *c, const wxImage &im) {
		c->text.image = g_sys->imagelist[g_sys->AddImageToList(im)];
	}

	bool Document::LoadImageIntoCell(const wxString &fn, Cell *c) {
	...
	void Document::ImageChange(wxString &fn) {
		selected.g->cell->AddUndo(this);
