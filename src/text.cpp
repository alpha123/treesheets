...
namespace treesheets {
	...
	wxBitmap *Text::DisplayImage() {
		return cell->grid && cell->grid->folded
			? &g_sys->frame->foldicon
			: (image ? &image->bm : nullptr);
	}

	size_t Text::EstimatedMemoryUse() {
	...
	void Text::WasEdited() { lastedit = wxDateTime::Now(); }
	int Text::MinRelsize(int rs) { return min(relsize, rs); }
	...
	bool Text::IsWord(wxChar c) { return wxIsalnum(c) || wxStrchr(L"_\"\'()", c); }
	wxString Text::GetLinePart(int &i, int p, int l) {
		int start = i;
		i = p;
	...
	wxString Text::GetLine(int &i, int maxcolwidth) {
		int l = (int)t.Len();
	...
	void Text::TextSize(wxDC &dc, int &sx, int &sy, bool tiny, int &leftoffset, int maxcolwidth) {
		sx = sy = 0;
		int i = 0;
		for (;;) {
	...
	bool Text::IsInSearch() { return g_sys->searchstring.Len() && t.Lower().Find(g_sys->searchstring) >= 0; }
	int Text::Render(Document *doc, int bx, int by, int depth, wxDC &dc, int &leftoffset,
		int maxcolwidth) {
		int ixs = 0, iys = 0;
		if (!cell->tiny) g_sys->ImageSize(DisplayImage(), ixs, iys);

		if (ixs && iys) {
			g_sys->ImageDraw(DisplayImage(), dc, bx + 1 + g_margin_extra,
	...
	void Text::FindCursor(Document *doc, int bx, int by, wxDC &dc, Selection &s, int maxcolwidth) {
		bx -= g_margin_extra;
		by -= g_margin_extra;
	...
	void Text::DrawCursor(Document *doc, wxDC &dc, Selection &s, bool full, uint color, bool cursoronly,
		int maxcolwidth) {
		int ixs = 0, iys = 0;
		if (!cell->tiny) g_sys->ImageSize(DisplayImage(), ixs, iys);
		if (ixs) ixs += 2;
	...
	void Text::SelectWord(Selection &s) {
	...
	void Text::SetRelSize() {
		if (t.Len() == 0 && cell->parent) cell->parent->grid->IdealRelSize(relsize);
	}

	void Text::Insert(Document *doc, const wxString &ins, Selection &s) {
	...
	void Text::Key(int k, Selection &s) {
		RangeSelRemove(s);
		SetRelSize();
	...
	void Text::ReplaceStr(const wxString &str) {
		for (int i = 0, j; (j = t.Mid(i).Lower().Find(g_sys->searchstring)) >= 0;) {
			// does this need WasEdited()?
			i += j;
			t.Remove(i, g_sys->searchstring.Len());
			t.insert(i, str);
			i += str.Len();
		}
	}
	...
	void Text::Save(wxDataOutputStream &dos) const {
	...
	void Text::Load(wxDataInputStream &dis) {
		t = dis.ReadString();

		// if (t.length() > 10000)
		//    printf("");

		if (g_sys->versionlastloaded <= 11) dis.Read32();  // numlines

		relsize = dis.Read32();

		int i = dis.Read32();
		image = i >= 0 ? g_sys->imagelist[g_sys->loadimageids[i]] : NULL;

		if (g_sys->versionlastloaded >= 7) stylebits = dis.Read32();

		wxLongLong time;
		if (g_sys->versionlastloaded >= 14) {
			dis.Read64(&time, 1);
		}
		else {
			time = g_sys->fakelasteditonload--;
		}
		lastedit = wxDateTime(time);
	}

	Cell *Text::Eval(Evaluator &ev) {
