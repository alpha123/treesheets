...
namespace treesheets {
	...
	void Grid::CloneStyleFrom(Grid *o) {
	...
	void Grid::InitColWidths() {
		...
		int cw = cell ? cell->ColWidth() : g_sys->defaultmaxcolwidth;
	...
	size_t Grid::EstimatedMemoryUse() {
		size_t sum = 0;
	...
	void Grid::Render(Document *doc, int bx, int by, wxDC &dc, int depth, int sx, int sy, int xoff, int yoff) {
		...
		if (tinyborder || cell->drawstyle == DS_GRID) {
			...
			dc.SetPen(view_grid_outer_spacing && !g_sys->fastrender ? g_sys->pen_gridlines
				: g_sys->pen_tinygridlines);
			drawlines();
		}
			...
			foreachcelly(c) if (c->HasContent() && !c->tiny) {
				...
				if (abs(srcy - desty) < arcsize && !cell->verticaltextandgrid) {
					...
					if (desty < srcy) {
						...
						if (visible) dc.DrawBitmap(g_sys->frame->line_nw, srcx, desty, true);
					}
					else {
						destylast = desty - arcsize;
						if (visible)
							dc.DrawBitmap(g_sys->frame->line_sw, srcx, desty - arcsize, true);
						desty--;
					}
	...
	void Grid::FindReplaceAll(const wxString &str) { foreachcell(c) c->FindReplaceAll(str); }
	void Grid::ReplaceCell(Cell *o, Cell *n) { foreachcell(c) if (c == o) c = n; }
	...
	void Grid::ImageRefCount() { foreachcell(c) c->ImageRefCount(); }
	...
	void Grid::DrawInsert(Document *doc, wxDC &dc, Selection &s, uint colour) {
		dc.SetPen(g_sys->pen_thinselect);
		if (!s.xs) {
	...
	void Grid::DelSelf(Document *doc, Selection &s) {
		if (!cell->parent || cell == doc->curdrawroot)
			return;  // FIXME, if drawroot, auto zoom out instead (needs dc)
	...
	void Grid::Save(wxDataOutputStream &dos) const {
		dos.Write32(xs);
		dos.Write32(ys);
	...
	bool Grid::LoadContents(wxDataInputStream &dis, int &numcells, int &textbytes) {
		if (g_sys->versionlastloaded >= 10) {
			bordercolor = dis.Read32() & 0xFFFFFF;
			user_grid_outer_spacing = dis.Read32();
			if (g_sys->versionlastloaded >= 11) {
				cell->verticaltextandgrid = dis.Read8() != 0;
				if (g_sys->versionlastloaded >= 13) {
					if (g_sys->versionlastloaded >= 16) {
						folded = dis.Read8() != 0;
						if (folded && g_sys->versionlastloaded <= 17) {
	...
	wxString Grid::ToText(int indent, const Selection &s, int format, Document *doc) {
	...
	wxString Grid::ConvertToText(const Selection &s, int indent, int format, Document *doc) {
		...
		Formatter(r, format, indent, L"<grid>\n",
			wxString::Format(
				L"<table border=1 cellspacing=0 cellpadding=3 style=\"font-size: %dpt;\">\n",
				12 - indent / 2)
			.wc_str());
	...
	static int sortfunc(const Cell **a, const Cell **b) {
		loop(i, g_sys->sortxs) {
			int off = (i + g_sys->sortcolumn) % g_sys->sortxs;
			int cmp = (*(a + off))->text.t.CmpNoCase((*(b + off))->text.t);
			if (cmp) return g_sys->sortdescending ? -cmp : cmp;
		}
		return 0;
	}

	void Grid::Sort(Selection &s, bool descending) {
		g_sys->sortcolumn = s.x;
		g_sys->sortxs = xs;
		g_sys->sortdescending = descending;
	...
	void Grid::CollectCells(Vector<Cell *> &itercells) { foreachcell(c) c->CollectCells(itercells); }
	...
	void Grid::ClearImages(Selection &s) { foreachcellinsel(c, s) c->text.image = NULL; }
};
