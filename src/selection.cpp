...
namespace treesheets {
	...
	bool Selection::IsAll() const { return xs == g->xs && ys == g->ys; }
	...
	Cell *Selection::GetCell() const { return g && xs == 1 && ys == 1 ? g->C(x, y) : NULL; }
	Cell *Selection::GetFirst() const { return g && xs >= 1 && ys >= 1 ? g->C(x, y) : NULL; }
	bool Selection::EqLoc(const Selection &s) {
		return g == s.g && x == s.x && y == s.y && xs == s.xs && ys == s.ys;
	}
	bool Selection::operator==(const Selection &s) {
	...
	bool Selection::IsInside(Selection &o) {
	...
	int Selection::MaxCursor() { return int(GetCell()->text.t.Len()); }
	char *Selection::Dir(Document *doc, bool ctrl, bool shift, wxDC &dc, int dx, int dy, int &v, int &vs,
	...
	char *Selection::Cursor(Document *doc, int k, bool ctrl, bool shift, wxDC &dc, bool exitedit) {
		switch (k) {
	...
	void Selection::Next(Document *doc, wxDC &dc, bool backwards) {
		doc->DrawSelect(dc, *this);
		ExitEdit(doc);
		if (backwards) {
			if (x > 0)
	...
	const char *Selection::Wrap(Document *doc) {
	...
	const char *Selection::HomeEnd(Document *doc, wxDC &dc, bool ishome) {
		doc->DrawSelect(dc, *this);
		xs = ys = 1;
		if (ishome)
			x = y = 0;
