...
namespace treesheets {
	...
	Cell::Cell(Cell *_p, Cell const *_clonefrom, int _ct, Grid *_g)
		: parent(_p),
		sx(0),
		sy(0),
		ox(0),
	...
	int Cell::MinRelsize()  // the smallest relsize is actually the biggest text
	{
		int rs = INT_MAX;
		if (grid) {
	...
	size_t Cell::EstimatedMemoryUse() {
		return sizeof(Cell) + text.EstimatedMemoryUse() + (grid ? grid->EstimatedMemoryUse() : 0);
	}

	void Cell::Layout(Document *doc, wxDC &dc, int depth, int maxcolwidth, bool forcetiny) {
		tiny = (text.filtered && !grid) || forcetiny ||
			doc->PickFont(dc, depth, text.relsize, text.stylebits);
		int ixs = 0, iys = 0;
		if (!tiny) g_sys->ImageSize(text.DisplayImage(), ixs, iys);
		int leftoffset = 0;
		if (!HasText()) {
			if (!ixs || !iys) {
	...
	void Cell::Render(Document *doc, int bx, int by, wxDC &dc, int depth, int ml, int mr, int mt, int mb,
		...
		if (drawstyle != DS_GRID && HasContent() && !tiny) {
			...
			if (drawstyle == DS_BLOBSHIER)
				dc.DrawRoundedRectangle(bx - cell_margin, by - cell_margin, minx + cell_margin * 2,
					miny + cell_margin * 2, g_sys->roundness);
			else if (HasHeader())
				dc.DrawRoundedRectangle(bx - cell_margin + g_margin_extra / 2,
					...
					tys + cell_margin * 2 + g_margin_extra, g_sys->roundness);
	...
	void Cell::CloneStyleFrom(Cell const *o) {
	...
	bool Cell::IsInside(int x, int y) const { return x >= 0 && y >= 0 && x < sx && y < sy; }
	int Cell::GetX(Document *doc) const { return ox + (parent ? parent->GetX(doc) : doc->hierarchysize); }
	int Cell::GetY(Document *doc) const { return oy + (parent ? parent->GetY(doc) : doc->hierarchysize); }
	int Cell::Depth() const { return parent ? parent->Depth() + 1 : 0; }
	Cell *Cell::Parent(int i) { return i ? parent->Parent(i - 1) : this; }
	Cell *Cell::SetParent(Cell *g) {
		parent = g;
		return this;
	}

	uint Cell::SwapColor(uint c) { return ((c & 0xFF) << 16) | (c & 0xFF00) | ((c & 0xFF0000) >> 16); }
	wxString Cell::ToText(int indent, const Selection &s, int format, Document *doc) {
	...
	void Cell::Reset() { ox = oy = sx = sy = minx = miny = 0; }
	...
	void Cell::LazyLayout(Document *doc, wxDC &dc, int depth, int maxcolwidth, bool forcetiny) {
		if (sx == 0) {
	...
	void Cell::Save(wxDataOutputStream &dos) const {
		dos.Write8(celltype);
	...
	Cell *Cell::LoadGrid(wxDataInputStream &dis, int &numcells, int &textbytes) {
		int xs = dis.Read32();
	...
	Cell *Cell::LoadWhich(wxDataInputStream &dis, Cell *_p, int &numcells, int &textbytes) {
		Cell *c = new Cell(_p, NULL, dis.Read8());
		numcells++;
		if (g_sys->versionlastloaded >= 8) {
			c->cellcolor = dis.Read32() & 0xFFFFFF;
			c->textcolor = dis.Read32() & 0xFFFFFF;
		}
		if (g_sys->versionlastloaded >= 15) c->drawstyle = dis.Read8();
		int ts;
		switch (ts = dis.Read8()) {
		case TS_BOTH:
		case TS_TEXT:
	...
	Cell *Cell::FindNextSearchMatch(wxString &search, Cell *best, Cell *selected, bool &lastwasselected) {
	...
	void Cell::FindReplaceAll(const wxString &str) {
		if (grid) grid->FindReplaceAll(str);
		text.ReplaceStr(str);
	}

	Cell *Cell::FindExact(wxString &s) { return text.t == s ? this : (grid ? grid->FindExact(s) : NULL); }
	void Cell::ImageRefCount() {
	...
	bool Cell::IsTag(Document *doc) { return doc->tags.find(text.t) != doc->tags.end(); }
	...
	int Cell::ColWidth() {
		return parent ? parent->grid->colwidths[parent->grid->FindCell(this).x]
			: g_sys->defaultmaxcolwidth;
	}

	void Cell::CollectCells(Vector<Cell *> &itercells, bool recurse) {
		itercells.push() = this;
